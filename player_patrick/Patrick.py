...
class Patrick:
    ...
    def __get_board_score(self, board):
        # print("\nmy:" + self.my_color + " - " + str(board.piece_count[self.my_color]))
    ...
    def min_max_max(self, a_board, max_depth):
        depth = max_depth - 1
        if (len(a_board.legal_moves(self.my_color)) > 0) & (depth > 0):  # se existe algum movimento possivel
    ...
    def min_max_min(self, a_board, max_depth):
        ...
        if (len(a_board.legal_moves(self.opponent_color)) > 0) & (
                depth > 0):  # se existe algum movimento possivel do oponente
...
if __name__ == '__main__':
    ...
    # f.write('%d,%d' % patrick.next_move_min_max(3))
